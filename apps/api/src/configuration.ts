...
export function validateConfiguration(
  ...
  for (const name of ["S3_SECRET_KEY", "INTERNAL_SERVICE_KEY"] as const) {
    ...
    if (value.length < 24 || /change-me|nap-log-dev/i.test(value))
      throw new Error(`${name} must be a strong production secret`);
  }
  const storageUrl = new URL(String(input.S3_PUBLIC_ENDPOINT));
  ...
  if (proofUrl.protocol !== "https:")
    throw new Error("PUBLIC_PROOF_BASE_URL must use HTTPS in production");
  return input;
}
