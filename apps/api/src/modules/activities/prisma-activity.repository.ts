import { ActivityStatus as PrismaStatus, Prisma } from "@prisma/client";
...
const statuses: Record<ActivityStatus, PrismaStatus> = {
  ...
  awaiting_assignment: PrismaStatus.AWAITING_ASSIGNMENT,
  assigned: PrismaStatus.ASSIGNED,
  accepted: PrismaStatus.ACCEPTED,
  en_route: PrismaStatus.EN_ROUTE,
  near_destination: PrismaStatus.NEAR_DESTINATION,
  on_site: PrismaStatus.ON_SITE,
  in_service: PrismaStatus.IN_SERVICE,
  completed: PrismaStatus.COMPLETED,
  failed: PrismaStatus.FAILED,
  rescheduled: PrismaStatus.RESCHEDULED,
  canceled: PrismaStatus.CANCELED,
...
export class PrismaActivityRepository implements ActivityRepository {
  ...
  async findAll(tenantId: string): Promise<Activity[]> {
    ...
    ).map((r) => this.map(r, r.assignments[0]?.driverId));
  }
  async findById(tenantId: string, id: string): Promise<Activity | undefined> {
    const r = await this.prisma.activity.findUnique({
    ...
    return r ? this.map(r, r.assignments[0]?.driverId) : undefined;
  }
  async save(a: Activity): Promise<Activity> {
    return this.prisma.$transaction(async (tx) => {
      const current = await tx.activity.findUniqueOrThrow({
        where: { tenantId_id: { tenantId: a.tenantId, id: a.id } },
      });
        ...
        data: {
          ...
          ...(a.assignedDriverId
            ? {
                assignments: {
                  create: {
                    tenant: { connect: { id: a.tenantId } },
                    driver: {
                      connect: {
                        tenantId_id: {
                          tenantId: a.tenantId,
                          id: a.assignedDriverId,
                        },
                      },
                    },
                  },
          ...
          events: {
            create: {
              ...
              payload: { from: current.status, to: a.status },
            },
          },
        },
      });
      return this.map(row, a.assignedDriverId);
    });
  }
  private map(
    r: {
      ...
      createdAt: Date;
      updatedAt: Date;
      events?: Array<{ payload: Prisma.JsonValue; occurredAt: Date }>;
    },
    driverId?: string,
  ): Activity {
    const event = r.events?.[0];
    let failure: Activity["failure"];
    if (
      event &&
      event.payload &&
    ...
    return {
      ...
      address: r.address,
      ...(r.destinationLatitude != null
        ? { destinationLatitude: r.destinationLatitude }
        : {}),
      ...(r.destinationLongitude != null
        ? { destinationLongitude: r.destinationLongitude }
        : {}),
      status: r.status.toLowerCase() as ActivityStatus,
