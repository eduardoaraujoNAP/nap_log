...
export class JwtAuthGuard implements CanActivate {
  ...
  constructor(
    ...
    this.bypass = config.get("DEV_AUTH_BYPASS", "false") === "true";
    if (this.bypass && env === "production")
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (this.bypass) {
      ...
      if (!id) throw new UnauthorizedException("Missing development tenant");
      const driverRaw = request.headers["x-driver-id"],
        driverId = Array.isArray(driverRaw) ? driverRaw[0] : driverRaw;
      this.tenant.set(id);
      request.user = {
        subject: "dev-bypass",
        tenantId: id,
        permissions: ["*"],
        claims: { ...(driverId ? { driver_id: driverId } : {}) },
      };
      return true;
    }
    ...
    if (!tenantId) throw new UnauthorizedException("Token has no tenant_id");
    const direct = Array.isArray(payload.permissions)
      ? payload.permissions.filter((v): v is string => typeof v === "string")
      : [];
    const realm = (payload.realm_access as { roles?: unknown } | undefined)
      ?.roles;
    const roles = Array.isArray(realm)
      ? realm.filter((v): v is string => typeof v === "string")
      : [];
    this.tenant.set(tenantId);
    request.user = {
      subject: payload.sub ?? "",
      tenantId,
      permissions: permissionsForRoles(direct, roles),
      claims: payload,
    };
    return true;
  }
