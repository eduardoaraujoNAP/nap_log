...
export function Shell({ children, devBypass = false }: { children: React.ReactNode; devBypass?: boolean }) {
  ...
  return <div className="app-shell">
    ...
    <div className="main-area">
      <header className="topbar"><div className="mobile-brand"><span className="brand-mark">N</span><b>NAPLOG</b></div><div className="top-search"><Icon name="search" size={18}/><input aria-label="Busca global" placeholder="Buscar atividade, motorista..."/><kbd>⌘ K</kbd></div><div className="top-actions"><button className="icon-button" aria-label="Notificações"><Icon name="bell"/><i/></button><div className="profile"><span>{status === "loading" ? "…" : initials}</span><div><b>{status === "loading" ? "Carregando sessão" : displayName}</b><small>{devBypass && !session ? "Autenticação demo" : session?.user?.email ?? "Sessão protegida"}</small></div><button className="session-action" onClick={() => session ? void signOut({ callbackUrl: "/login" }) : void signIn("keycloak")}>{session || devBypass ? "Sair" : "Entrar"}</button></div></div></header>
      <main>{children}</main>
