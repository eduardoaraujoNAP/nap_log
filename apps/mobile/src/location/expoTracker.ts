...
TaskManager.defineTask(LOCATION_TASK, async ({ data, error }) => {
  ...
  const journeyId = await store.getMetadata("activeJourneyId");
  if (!journeyId) return;
  const locations =
    (data as { locations?: Location.LocationObject[] }).locations ?? [];
  const points = locations.map((value) => mapPoint(journeyId, value));
  await store.saveLocationPoints(points);
  const latest = points.at(-1);
  if (latest) await store.setMetadata("latestLocation", JSON.stringify(latest));
});
export class ExpoLocationTracker implements LocationTracker {
  constructor(
    private readonly store: LocalStore,
    private readonly policy: TrackingPolicy = DEFAULT_TRACKING_POLICY,
  ) {}
  async startJourney(journeyId: string): Promise<TrackerStartResult> {
    const foreground = await Location.requestForegroundPermissionsAsync();
    if (foreground.status !== "granted")
      return { permission: "foreground-denied", active: false };
    const background = await Location.requestBackgroundPermissionsAsync();
    if (background.status !== "granted")
      return { permission: "background-denied", active: false };
    await this.store.setMetadata("activeJourneyId", journeyId);
    if (!(await Location.hasStartedLocationUpdatesAsync(LOCATION_TASK)))
      await Location.startLocationUpdatesAsync(LOCATION_TASK, {
        accuracy: Location.Accuracy.Balanced,
        timeInterval: this.policy.timeIntervalMs,
        distanceInterval: this.policy.distanceIntervalM,
        pausesUpdatesAutomatically: false,
        showsBackgroundLocationIndicator: true,
        foregroundService: {
          notificationTitle: "Jornada em andamento",
          notificationBody: "O NAP Log registra sua rota durante a jornada.",
  ...
  async stopJourney() {
    await this.store.setMetadata("activeJourneyId", "");
    if (await Location.hasStartedLocationUpdatesAsync(LOCATION_TASK))
