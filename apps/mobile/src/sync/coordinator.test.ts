...
describe('sync coordinator', () => {
  it('acknowledges only applied/duplicate results and retains conflicts', async () => {
    ...
    const request = vi.fn(async () => new Response(JSON.stringify([{ clientCommandId: applied.id, status: 'applied' }, { clientCommandId: conflict.id, status: 'conflict' }]), { status: 200 }));
  ...
  it('does not acknowledge a partial response missing a command', async () => {
    ...
    const request = vi.fn(async () => new Response(JSON.stringify([{ clientCommandId: first.id, status: 'duplicate' }]), { status: 200 }));
