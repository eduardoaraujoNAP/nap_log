...
export interface ApiSyncConfig { apiUrl: string; deviceId: string; accessToken?: string; devAuthBypass: boolean; tenantId?: string; driverId?: string; }
...
export class ApiSyncClient {
  ...
  async listActivities(): Promise<Activity[]> { return this.json('/mobile/activities', { method: 'GET' }); }
  ...
  private async json<T>(path: string, init: RequestInit): Promise<T> { const response = await this.request(`${this.config.apiUrl.replace(/\/$/, '')}${path}`, { ...init, headers: { 'content-type': 'application/json', ...(this.config.accessToken ? { authorization: 'Bearer ' + this.config.accessToken } : {}), ...(this.config.devAuthBypass && this.config.tenantId ? { 'x-tenant-id': this.config.tenantId } : {}), ...(this.config.devAuthBypass && this.config.driverId ? { 'x-driver-id': this.config.driverId } : {}), ...init.headers } }); if (!response.ok) throw new ApiRequestError(response.status, 'API ' + response.status); return response.json() as Promise<T>; }
}
export function apiConfigFromEnv(accessToken: string | undefined, deviceId: string): ApiSyncConfig | undefined {
  const env = process.env; const apiUrl = env.EXPO_PUBLIC_API_URL; const devAuthBypass = typeof __DEV__ !== 'undefined' && __DEV__ && env.EXPO_PUBLIC_DEV_AUTH_BYPASS === 'true'; const tenantId = devAuthBypass ? env.EXPO_PUBLIC_TENANT_ID ?? env.TENANT_ID : undefined; const driverId = devAuthBypass ? env.EXPO_PUBLIC_DRIVER_ID : undefined;
  return apiUrl && (accessToken || (devAuthBypass && tenantId && driverId)) ? { apiUrl, deviceId, accessToken, devAuthBypass, tenantId, driverId } : undefined;
}
