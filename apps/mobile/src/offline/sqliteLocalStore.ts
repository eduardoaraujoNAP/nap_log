...
export class SQLiteLocalStore implements LocalStore {
  ...
  async replaceActivities(activities: Activity[]) { await this.db.withTransactionAsync(async () => { await this.db.runAsync('DELETE FROM activities'); const now = new Date().toISOString(); for (const item of activities) await this.db.runAsync('INSERT INTO activities (id, body, updated_at) VALUES (?, ?, ?)', item.id, serializeActivity(item), now); }); }
